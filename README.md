...
- 依典藏機構篩選，共 36 件精選公共領域／CC0 作品，每館 12 件。
...
作品資料是 2026-07-16 核對的靜態策展快照，放在 `src/data/catalog.json`；繁體中文策展題名與作品清單定義在 `scripts/sync-artworks.mjs`。採用靜態快照而非每次開站都即時搜尋 API，目的是避免搜尋排序、限流或上游短暫異常影響展覽順序；每件作品仍直接連結原始 API JSON，可隨時核對最新紀錄。

為避免 Brave 等瀏覽器的隱私防護、跨站 Cookie 或上游短暫異常造成空白影像，網站使用由官方開放影像製作的顯示用 JPG 副本，存放於 `public/artworks/`。這些副本只用於穩定顯示，不取代原始來源；每件作品的來源面板與下方清單仍保留官方館藏頁、API JSON 與原始影像連結。

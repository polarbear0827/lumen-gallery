...
「光室」是一個可全螢幕瀏覽的數位畫廊，只收錄 The Metropolitan Museum of Art、Art Institute of Chicago 與 Cleveland Museum of Art 三間博物館的開放典藏繪畫與攝影。
...
- 只允許 API 類型為 Painting 或 Photograph 的作品；雕塑、瓷器、織物、版畫、家具、器皿及其他物件一律排除。
- 開站後直接向三館 API 載入至少 1,000 件合格作品；2026-07-17 正式驗證可載入超過 1,000 件繪畫。
- 內建 35 件已核對且可離線顯示的繪畫備援；它們是啟動畫面與上游異常時的保底，不是完整館藏。
- 可主動開啟「輪播模式」，每 6 秒自動切換一幅作品，並可隨時暫停。
...
1. `src/data/catalog.json` 是 2026-07-16 核對的本地策展快照；其中版畫《神奈川沖浪裏》已從播放資料流排除，因此實際備援為 35 件繪畫。繁體中文策展題名與同步清單定義在 `scripts/sync-artworks.mjs`。
...
開站時三館並行載入。Cleveland 直接以官方 `type=Painting`、`cc0`、`has_image=1` 條件取得 1,000 件；使用者瀏覽到目前清單最後 10 件內時，再向目前機構取得下一批。資料只存在目前瀏覽器記憶體，不會把數千張影像複製到 repo，也不會寫入 Cookie 或遠端資料庫。

三館的即時篩選方式如下：

- The Met：先呼叫 `search?hasImages=true&q=*` 取得候選 ID，再逐件讀取資料，只保留 `isPublicDomain: true`、有官方影像，且 `objectName`／`classification` 為 Painting 或 Photograph 者。
- 芝加哥藝術博物館：從官方 artworks 分頁端點每次讀取 100 筆，再於前端只保留 `is_public_domain: true`、有 `image_id`，且 `artwork_type_title` 為 Painting 或 Photograph 者。這個方式避開複合搜尋 POST 在部分瀏覽器的跨來源 403。
- Cleveland Museum of Art：API 查詢直接加入 `type=Painting`、`cc0` 與 `has_image=1`，並再次核對每筆 `type`、`share_license_status` 與官方影像。

介面只顯示通過上述條件的「已載入」數量，不再以含有其他器物的上游候選總數充當館藏數。

為避免 Brave 等瀏覽器的隱私防護、跨站限制或上游短暫異常造成啟動畫面空白，35 件備援繪畫使用由官方開放影像製作的顯示用 JPG 副本，存放於 `public/artworks/`。API 動態作品直接使用博物館官方影像網址；若影像請求失敗，該筆會自動從目前輪播排除，不留下空白畫框。每件可播放作品的來源面板都保留作品類型、官方館藏頁、API JSON、原始影像、授權與影像署名。
...
下表只列出 repo 內 35 件會進入畫廊的本地繪畫備援，不代表即時 API 輪播上限。中文題名是本站為繁體中文介面所做的編輯翻譯，並非典藏機構官方譯名。本站顯示用 JPG 副本位於 `public/artworks/`，均由下表官方開放來源製作；動態作品的完整資料與來源則在網站的「作品資料與來源」面板依該次 API 回應即時揭露。
